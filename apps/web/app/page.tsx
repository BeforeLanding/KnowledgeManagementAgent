...
type OperationsStatus = {
  release: string;
  readiness: { status: "ready" | "not_ready"; components: Record<string, { ready: boolean; latency_ms: number; code: string }> };
  evaluation_gate: { suite: string; version: string; status: string; gate_passed: boolean | null; completed_at: string | null } | null;
  security_summary: { window_hours: number; failed_agent_runs: number; refusals: number; postgres_authoritative: boolean; details_redacted: boolean };
};
...
export default function Home() {
  ...
  async function load(accessToken = token) {
    ...
    const [spaceData, documentData, traceData, suiteData, runData, operationsData] = await Promise.all([
      request("/api/v1/spaces", accessToken), request("/api/v1/documents", accessToken), request("/api/v1/traces", accessToken), request("/api/v1/evaluations/suites", accessToken), request("/api/v1/evaluations/runs", accessToken), request("/api/v1/operations/status", accessToken),
    ]);
    setSpaces(spaceData); setDocuments(documentData); setTraces(traceData); setEvaluationSuites(suiteData); setEvaluationRuns(runData); setOperations(operationsData);
  }
  ...
  return (
    <main className="shell">
      ...
      <section className="content">
        <header><div><span className="eyebrow">ENTERPRISE KNOWLEDGE</span><h1>{tab === "chat" ? "Ask with confidence" : tab === "knowledge" ? "Knowledge library" : "Quality control"}</h1></div><span className={`health ${operations?.readiness.status === "not_ready" ? "not-ready" : ""}`}>● {operations?.readiness.status === "ready" ? "System ready" : operations ? "Dependencies degraded" : "Checking status"}</span></header>
